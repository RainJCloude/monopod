...
class JointAction(ActionTerm):
    ...
    def process_actions_adding_the_trajectory(self, actions: torch.Tensor):
        ...
        self._offset= torch.column_stack((val_m1_for_env,
                                            val_m2_for_env,
                                            torch.zeros(self.env.num_envs, device=self.env.device))).clone()

        self._processed_actions = self._raw_actions * self._scale + self._offset

  
        self.previous_act = self._processed_actions[0][0].clone()
